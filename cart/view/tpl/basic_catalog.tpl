<div class="generic-content-wrapper dm42cart catalog">
	<div class="section-title-wrapper clearfix">
		{{if $total_qty}}
		<a href="cart/{{$sellernick}}/checkout/start" class="btn btn-sm btn-success float-end"><i class="bi bi-cart"></i> Checkout ({{$total_qty}})</a>
		{{/if}}
	...
	<div class="section-content-wrapper">
		<table class="w-100">
			...
			<tr>
				...
				<td>{{if $item.order_qty}}<i class="bi bi-cart"></i> {{$item.order_qty}}{{/if}}</td>
				<td>
					{{if $item.order_qty}}
					<form method="post">
						<input type="hidden" name="cart_posthook" value="update_item">
						<input type="hidden" name="delsku" value="{{$item.item_sku}}">
						<button class="btn btn-outline-danger btn-outline border-0" type="submit" name="remove" title="Remove from cart"><i class="bi bi-x-lg"></i></button>
					</form>
					{{/if}}
				</td>
			</tr>
