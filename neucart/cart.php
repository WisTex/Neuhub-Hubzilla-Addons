...
class Cart {
	...
	public static function set_seller_by_hash($channel_hash) {
		Cart::$seller = [];
		$params       = ["channel_id", "channel_account_id", "channel_primary", "channel_name", "channel_address", "channel_hash", "channel_timezone"];
	...
	public static function set_buyer_by_hash($channel_hash) {
		...
		if ($channel) {
			...
			$params  = ["channel_id" => "channel_id", "channel_account_id" => "channel_account_id", "channel_primary" => "channel_primary", "channel_timezone" => "channel_timezone", "hash" => "channel_hash", "address" => "channel_address", "name" => "channel_name"];
			$channel = xchan_fetch(["hash" => $channel_hash]);
			if ($channel) {
...
function cart_additem_hook(&$hookdata) {
	...
	$keys = [
		"order_hash"    => ["key" => "order_hash", "cast" => "'%s'", "escfunc" => "dbesc"],
		"item_desc"     => ["key" => "item_desc", "cast" => "'%s'", "escfunc" => "dbesc"],
		"item_type"     => ["key" => "item_type", "cast" => "'%s'", "escfunc" => "dbesc"],
		"item_sku"      => ["key" => "item_sku", "cast" => "'%s'", "escfunc" => "dbesc"],
		"item_qty"      => ["key" => "item_qty", "cast" => "%d", "escfunc" => "intval"],
		"item_price"    => ["key" => "item_price", "cast" => "%f", "escfunc" => "floatval"],
		"item_tax_rate" => ["key" => "item_tax_rate", "cast" => "%f", "escfunc" => "floatval"],
		"item_meta"     => ["key" => "item_meta", "cast" => "'%s'", "escfunc" => "dbesc"],
	];
...
function cart_do_additem(&$hookdata) {
	...
	$required       = ["item_sku", "item_qty", "item_desc", "item_price"];
...
function cart_updateitem_hook(&$hookdata) {
	...
	$string_components  = ["item_sku", "item_desc"];
	$int_components     = ["item_qty", "item_confirmed", "item_fulfilled", "item_exception"];
	$decimal_components = ["item_price", "item_tax_rate"];
...
function cart_do_updateitem(&$hookdata) {
	...
	$calldata = ['order' => $order, 'item' => $iteminfo];
...
function cart_getitemtypes() {
	$itemtypes = cart_getsysconfig("itemtypes");
	$itemtypes = is_array($itemtypes) ? $itemtypes : [];
	return $itemtypes;
}
...
function cart_do_checkout_before(&$hookdata) {
	...
	foreach ($order["items"] as $iteminfo) {
		...
		$calldata = ['itemid' => $iteminfo, 'error' => null, 'content' => $hookdata["content"]];
...
function cart_do_orderpaid(&$hookdata) {
	...
	$order          = cart_loadorder($orderhash);
	$cart_itemtypes = cart_getitemtypes();
	$payment        = isset($hookdata["payment"]) ? $hookdata["payment"] : [];
	...
	foreach ($order["items"] as $iteminfo) {
		...
		$calldata = ['item' => $iteminfo, 'error' => null, 'content' => null];
		$itemtype = isset($calldata['item']['item_type']) ? $calldata['item']['item_type'] : null;
...
function cart_install() {
	...
	cart_setsysconfig("appver", Cart::$cart_version);
...
function cart_get_test_catalog(&$items) {
	...
	$items = array_merge($items, [
		"sku-1" => ["item_sku" => "sku-1", "item_desc" => "Description Item 1", "item_price" => 5.55],
		"sku-2" => ["item_sku" => "sku-2", "item_desc" => "Description Item 2", "item_price" => 6.55],
		"sku-3" => ["item_sku" => "sku-3", "item_desc" => "Description Item 3", "item_price" => 7.55],
		"sku-4" => ["item_sku" => "sku-4", "item_desc" => "Description Item 4", "item_price" => 8.55],
		"sku-5" => ["item_sku" => "sku-5", "item_desc" => "Description Item 5", "item_price" => 9.55],
		"sku-6" => ["item_sku" => "sku-6", "item_desc" => "Description Item 6", "item_price" => 10.55]
	]);
...
function cart_fulfillitem_error($error, $itemid, $orderhash) {
	...
	if ($exception) {
		q("update cart_orderitems set item_exception = 1 where order_hash = '%s' and id = %d",
			dbesc($orderhash),
			intval($itemid)
		);
